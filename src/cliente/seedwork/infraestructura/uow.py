...
def registrar_unidad_de_trabajo(serialized_obj):
    from entregasDeLosAlpes.config.uow import UnidadTrabajoSQLAlchemy
    from flask import session
...
def flask_uow():
    from flask import session
    from entregasDeLosAlpes.config.uow import UnidadTrabajoSQLAlchemy, UnidadTrabajoPulsar
    if session.get('uow'):
        return session['uow']
